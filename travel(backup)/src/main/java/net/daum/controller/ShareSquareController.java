...
public class ShareSquareController {
	...
	//후기 작성 완료하면 공유화면으로 받을꺼임@RequestMapping(value="/plan_share", method=RequestMethod.GET)
	
	//작성 후기 저장
	@GetMapping("/ShareSquare_List")
	public ModelAndView ShareSquare_list(ShareSquareVO s, HttpServletResponse response, HttpSession session) throws Exception {
		response.setContentType("text/html;charset=UTF-8");
	
		//리스트 불러오기
		List<ShareSquareVO> slist = this.sharesquareService.getshareSquareList(s);
	
		ModelAndView mav=new ModelAndView();
		mav.addObject("share",slist);
		mav.setViewName("jsp/ShareSquare_list");
		return mav;
		}
	...
	/*@GetMapping("/ShareSquare_List")
	public ModelAndView ShareSquare_list(ShareSquareVO s, PageVO p, HttpServletRequest request,
